...
const generators = groupToGenerator["A5"];
let newGraph = {
  nodes: [],
  edges: [],
};
...
function DFS(currentNode, nGraph, generators) {
  ...
  for (const gen of generators) {
    const child = mult([...gen, ...currentNode], 4);
...
go down left, go down left, ... when you hit a node that's already in the graph, 
pop and go right, then rep

def DFS(current node):
  for each generator:
    nextNode

def pushdown(current node, generator):
  returns ndoe
